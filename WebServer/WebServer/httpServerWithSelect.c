...
#include "httpServerWithSelect.h"
        ...
        for(int i = 0; i <= fdmax; i++)
        {
            if(FD_ISSET(i, &tempreadfds))
            {
                ...
                if(i == cxSocket)
                {
                    ...
                    params->fd = socketNewCx;
                    params->file_path = arguments->file_path;
                    
                    printf("\nAdding thread to pool -> file descriptor of the connection: %d\n", params->fd);
                ...
                {
                    char buffer[256];
                    ssize_t ret = recv(i, &buffer, 256, 0);
